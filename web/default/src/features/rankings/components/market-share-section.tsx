    ...
    <ul>
      {props.rows.map((vendor) => (
        <li key={vendor.vendor} className='flex items-center gap-3 py-2.5'>
          ...
          <span
            aria-hidden
            className='flex size-7 shrink-0 items-center justify-center rounded-full border border-border/40 bg-background'
            style={{
              color: props.colourMap[vendor.vendor] ?? '#94a3b8',
            }}
          >
