...
export function Features(_props: FeaturesProps) {
  ...
  const features = [
      ...
      visual: (
        <div className='mt-4 grid grid-cols-3 gap-2'>
          {modelBadges.map((model) => (
              <div
                key={model.name}
                className='border-border/30 bg-muted/20 text-muted-foreground flex min-w-0 items-center justify-center gap-1.5 rounded-lg border px-2.5 py-2 text-xs transition-colors duration-300 hover:border-blue-500/30 hover:bg-blue-500/5'
              >
    ...
    },
    {
      ...
      num: '04',
      title: '模型接入友好',
      desc: '常用客户端和自动化工具可快速接入模型服务',
      span: 'md:col-span-2',
      icon: <Code className='size-4 text-amber-400' />,
      visual: (
        <div className='mt-4 flex items-center gap-3'>
          <div className='flex -space-x-2'>
            {['Chat', 'Claude', 'Gemini', 'GPT'].map((n) => (
              <div
                key={n}
